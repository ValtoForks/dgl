bl_info = {
    ...
    "version": (1, 0),
    "blender": (2, 6, 4),
    "location": "File > Export > DGL Scene (.dgl)",
...
def writeEntity(f, obj, entityId, meshes, materials):
    ...
    data = data + struct.pack('<I', jsonSize)
    data = data + bytearray(jsonASCII)
...
class ExportDGLFile(bpy.types.Operator, ExportHelper):
    bl_idname = "export_objects.dgl"
    bl_label = "Export DGL"
    filename_ext = ".dgl"

    filter_glob = StringProperty(default="unknown.dgl", options={'HIDDEN'})
...
def menu_func_export_dgl(self, context):
    self.layout.operator(ExportDGLFile.bl_idname, text = "DGL Scene (.dgl)")
